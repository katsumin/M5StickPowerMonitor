...
{
public:
    virtual void init(unsigned int port = 3610)
    {
        _pUdp = new WiFiUDP();
        _localPort = port;
        _active = true;
    ...
    void getServer()
    {
        ...
        // WiFiUDP *udp = (WiFiUDP *)getUdp();
        // uint8_t r = udp->beginMulticast(IPAddress(224, 0, 23, 0), 3610); // Echonet requests are to port 3610
        uint8_t r = udp->beginPacket(IPAddress(224, 0, 23, 0), 3610); // Echonet requests are to port 3610
        // uint8_t r = udp->beginMulticastPacket(); // Echonet requests are to port 3610
        // Serial.print(r);
        if (r == 1)
        {
...
private:
    void static _task(void *arm)
    {
        ...
        // uint8_t r = ((WiFiUDP *)pUdp)->beginMulticast(IPAddress(224, 0, 23, 0), own->_localPort);
        if (r == 1)
        {
